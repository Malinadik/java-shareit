...
public class BookingControllerTests {
    ...
    public void beforeEach() {
        objectMapper = new ObjectMapper().configure(SerializationFeature.WRITE_DATES_AS_TIMESTAMPS, false).setDateFormat(new StdDateFormat().withColonInTimeZone(true));
    ...
    public void testAddBooking() throws Exception {
        ...
        Long userId = 1L;
        BookingEntryDto bookingDto = BookingEntryDto.builder().id(1L).itemId(1L).start(LocalDateTime.now().plusHours(1)).end(LocalDateTime.now().plusHours(2)).build();
        BookingDto bookingResponse = BookingDto.builder().id(1L).item(BookingDto.Item.builder().id(1L).build()).booker(BookingDto.User.builder().id(1L).build()).status(Status.WAITING).build();
        when(bookingService.addBooking(anyLong(), ArgumentMatchers.any(BookingEntryDto.class))).thenReturn(bookingResponse);
        mockMvc.perform(post("/bookings").header("X-Sharer-User-Id", userId).content(objectMapper.writeValueAsString(bookingDto)).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk()).andExpect(jsonPath("$.id", is(bookingResponse.getId().intValue()))).andExpect(jsonPath("$.item.id", is(bookingResponse.getItem().getId().intValue()))).andExpect(jsonPath("$.booker.id", is(bookingResponse.getBooker().getId().intValue()))).andExpect(jsonPath("$.status", equalTo(bookingResponse.getStatus().toString())));
    }
    ...
    public void testAddBooking_InvalidUserId() throws Exception {
        ...
        BookingEntryDto bookingDto = BookingEntryDto.builder().id(1L).itemId(1L).start(LocalDateTime.now().plusHours(1)).end(LocalDateTime.now().plusHours(2)).build();
        when(bookingService.addBooking(anyLong(), ArgumentMatchers.any(BookingEntryDto.class))).thenThrow(new NotFoundException("User not found."));
        mockMvc.perform(post("/bookings").header("X-Sharer-User-Id", invalidUserId).content(objectMapper.writeValueAsString(bookingDto)).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isNotFound());
    }

    /*@Test
    void getAllBooking_InvalidState() {
        assertThrows(NotSupportedStateException.class, () -> bookingController.getAllBookingByState(1L, "NOTSUPP", 0, 10));
    }*/
    ...
    public void testApproveBooking() throws Exception {
        ...
        BookingDto bookingResponse = BookingDto.builder().id(1L).item(BookingDto.Item.builder().id(1L).build()).booker(BookingDto.User.builder().id(1L).build()).start(LocalDateTime.now()).end(LocalDateTime.now().plusHours(1)).status(Status.APPROVED).build();
    ...
    public void testGetBookingById() throws Exception {
        Long userId = 1L;
        Long bookingId = 1L;
        BookingDto bookingResponse = BookingDto.builder().id(1L).item(BookingDto.Item.builder().id(1L).build()).booker(BookingDto.User.builder().id(1L).build()).start(LocalDateTime.now()).end(LocalDateTime.now().plusHours(1)).status(Status.WAITING).build();
    ...
    public void testGetAllBookingByState() throws Exception {
        ...
        int from = 0;
        int size = 10;
        BookingDto booking1 = BookingDto.builder().id(1L).item(BookingDto.Item.builder().id(1L).build()).booker(BookingDto.User.builder().id(1L).build()).start(LocalDateTime.now()).end(LocalDateTime.now().plusHours(1)).status(Status.WAITING).build();
        List<BookingDto> bookingList = Collections.singletonList(booking1);
        when(bookingService.getAllBookingByState(anyLong(), ArgumentMatchers.any(State.class), anyInt(), anyInt())).thenReturn(bookingList);
    ...
    public void testGetAllBookingByState_InvalidPageFrom() throws Exception {
        ...
        int from = -1;
        int size = 10;
        when(bookingService.getAllBookingByState(anyLong(), ArgumentMatchers.any(State.class), anyInt(), anyInt())).thenThrow(new ValidationException(""));
        mockMvc.perform(get("/bookings")
                        .header("X-Sharer-User-Id", invalidUserId)
                        .param("state", state).param("from",
                                String.valueOf(from)).param("size", String.valueOf(size)))
    ...
    public void testGetAllBookingByState_InvalidPageSize() throws Exception {
        ...
        mockMvc.perform(get("/bookings")
                ...
                .param("size", String.valueOf(size))).andExpect(status().isInternalServerError());
    }
    ...
    public void testGetAllBookingByState_InvalidUserId() throws Exception {
        ...
        when(bookingService.getAllBookingByState(anyLong(), ArgumentMatchers.any(State.class), anyInt(), anyInt())).thenThrow(new NotFoundException("User not found."));
    ...
    public void testGetAllItemsBookings() throws Exception {
        ...
        BookingDto booking2 = BookingDto.builder().id(2L).item(BookingDto.Item.builder().id(2L).build()).booker(BookingDto.User.builder().id(2L).build()).start(LocalDateTime.now()).end(LocalDateTime.now().plusHours(1)).status(Status.APPROVED).build();

        List<BookingDto> bookingList = Arrays.asList(booking1, booking2);

        when(bookingService.getAllOwnersBookingByState(anyLong(), ArgumentMatchers.any(State.class), anyInt(), anyInt())).thenReturn(bookingList);
    ...
    public void testGetAllItemsBookings_InvalidUserId() throws Exception {
        ...
        when(bookingService.getAllOwnersBookingByState(anyLong(), ArgumentMatchers.any(State.class), anyInt(), anyInt())).thenThrow(new NotFoundException("User not found."));
    ...
    public void testGetAllItemsBookings_InvalidPageFrom() throws Exception {
        ...
        when(bookingService.getAllOwnersBookingByState(anyLong(), ArgumentMatchers.any(State.class), anyInt(), anyInt())).thenThrow(new ValidationException(""));

        mockMvc.perform(get("/bookings/owner").header("X-Sharer-User-Id", invalidUserId).param("state", state).param("from", String.valueOf(from)).param("size", String.valueOf(size))).andExpect(status().isInternalServerError());
    }
